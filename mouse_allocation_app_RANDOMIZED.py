...
if uploaded_file:
    ...
    treatment_counts = {"Vehicle": 0, "Drug A": 0}
    ...
    def assign_group(row):
        ...
        for treatment in eligible_treatments:
            ...
            # Add slight penalty to treatment with more total mice
            penalty = treatment_counts[treatment] * 0.01
            total_score = deviation + penalty

            scores.append((total_score, group, treatment))

        if not scores:
            return None, None

        # Pick group with lowest score, tiebreak randomly
        scores.sort()
        best_score = scores[0][0]
        best_choices = [s for s in scores if abs(s[0] - best_score) < 1e-6]
        _, best_group, best_treatment = random.choice(best_choices)
